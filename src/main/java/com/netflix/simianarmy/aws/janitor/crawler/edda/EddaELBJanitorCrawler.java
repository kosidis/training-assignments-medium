...
public class EddaELBJanitorCrawler implements JanitorCrawler {
    ...
    private Map<String, List<DNSEntry>> buildELBtoDNSMap(String region) {
        String url = String.format("%s/aws/hostedRecords;_expand:(name,type,aliasTarget,resourceRecords:(value),zone:(id))", eddaClient.getBaseUrl(region));
        ...
        return new DNSMapBuilder(jsonNode).build();
    }

    private String getHostedZoneId(JsonNode dnsNode) {
        if (dnsNode.get("zone") != null) {
            JsonNode hostedZoneIdNode = dnsNode.get("zone").get("id");
    ...
    private class DNSMapBuilder {
        private JsonNode jsonNode;
        private final Map<String, List<DNSEntry>> dnsMap = new HashMap<>();

        public DNSMapBuilder(JsonNode jsonNode) {
            this.jsonNode = jsonNode;
        }

        public Map<String, List<DNSEntry>> build() {
            for (Iterator<JsonNode> it = jsonNode.getElements(); it.hasNext(); ) {
                JsonNode dnsNode = it.next();

                DNSEntry dnsEntry = createDnsEntry(dnsNode.get("name").getTextValue(), dnsNode.get("type").getTextValue(), getHostedZoneId(dnsNode));

                addAliasTarget(dnsNode, dnsEntry);

                addResourceRecords(dnsNode, dnsEntry);
            }

            return dnsMap;
        }

        private void addResourceRecords(JsonNode dnsNode, DNSEntry dnsEntry) {
            ...
            if (records != null && records.isArray() && records.size() != 0) {
                for (Iterator<JsonNode> recordNode = records.getElements(); recordNode.hasNext(); ) {
                    ...
                    if (elbDNS.contains(".elb.")) {
                        ...
                        List<DNSEntry> dnsEntryList = new GetValue(elbDNS).invoke();

                        dnsEntryList.add(dnsEntry);
                        LOGGER.debug(String.format("Found DNS %s associated with ELB DNS %s, type %s, zone %s", dnsNode.get("name").getTextValue(), elbDNS, dnsNode.get("type").getTextValue(), getHostedZoneId(dnsNode)));
                    }
                }
            }
        }

        private void addAliasTarget(JsonNode dnsNode, DNSEntry dnsEntry) {
            ...
            if (aliasTarget != null) {
                ...
                if (aliasTargetDnsNameNode != null) {
                    ...
                    if (aliasTargetDnsName != null && aliasTargetDnsName.contains(".elb.")) {
                        ...
                        List<DNSEntry> dnsEntryList = new GetValue(aliasTargetDnsName).invoke();

                        dnsEntryList.add(dnsEntry);
                        LOGGER.debug(String.format("Found DNS %s (alias) associated with ELB DNS %s, type %s, zone %s", dnsNode.get("name").getTextValue(), aliasTargetDnsName, dnsNode.get("type").getTextValue(), getHostedZoneId(dnsNode)));
                    }
                }
            }
        }
        ...
        private class GetValue {
            ...
            public GetValue(String aliasTargetDnsName) {
                this.aliasTargetDnsName = aliasTargetDnsName;
            }

            public List<DNSEntry> invoke() {
